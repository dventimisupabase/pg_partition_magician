...
create temporary table _pk_before as
  select i.indexrelid::oid as idx from pg_index i
   where i.indrelid = 'public.multi'::regclass and i.indisprimary;
call pgpm.transmute('public.multi', 'id', 100000);
select pass('transmute accepts a composite PK partitioned by a non-leading member of it');
select is(
...
select throws_like(
  $$ call pgpm.transmute('public.nopk', 'id', 100000) $$,
...
select throws_ok(
  $$ call pgpm.transmute('public.idpk', 'created_at', interval '1 month') $$,
  'P0001', NULL,
