...
-- A naive value has no zone of its own, so its grid is computed on the column's OWN wall clock, which is
-- the UTC lattice: partition_tz is recorded as UTC for such a column whatever the session's zone (#504).
-- The same table therefore gets the same bounds whichever session builds them: the monolith (built under
-- New York) and the forward children (built under UTC) must meet on the same wall-clock literal, and the
-- boundaries are midnight on the 1st on the column's own clock.
set timezone = 'America/New_York';
...
select is((select string_agg(parent_table::text || '=' || partition_tz, ',' order by parent_table::text) from pgpm.config
            where parent_table in ('public.gt_n'::regclass, 'public.gt_d'::regclass)),
  'gt_d=UTC,gt_n=UTC', 'both naive columns record UTC although the transmuting session is in New York: their grid is their own wall clock');
set timezone = 'UTC';
...
select is(
  ...
  format('FOR VALUES FROM (%L) TO (%L)',
         ...
         to_char(date_trunc('month', now() at time zone 'UTC') + interval '1 month', 'YYYY-MM-DD HH24:MI:SS')),
  'timestamp column: the monolith (built under New York) is bounded by month starts on the column''s own clock, from the oldest row''s month to the month after the frontier''s');
select is(
  (select string_agg(bound, '; ' order by lo) from bounds
    where parent_table = 'public.gt_n'::regclass
      and lo >= (date_trunc('month', now() at time zone 'UTC') + interval '1 month') at time zone 'UTC'),
  (select string_agg(format('FOR VALUES FROM (%L) TO (%L)', to_char(m, 'YYYY-MM-DD HH24:MI:SS'),
     ...
     from generate_series(date_trunc('month', now() at time zone 'UTC') + interval '1 month',
                          date_trunc('month', (now() at time zone 'America/New_York') + interval '300 days'),
                          interval '1 month') m),
  'timestamp column: the forward children (built under UTC) carry first-of-month literals and meet the monolith exactly');
...
select is(
  ...
  format('FOR VALUES FROM (%L) TO (%L)',
         ...
         to_char(date_trunc('month', now() at time zone 'UTC') + interval '1 month', 'YYYY-MM-DD')),
  'date column: the monolith is bounded by the first of the month, as the column''s own calendar counts months');
select is(
  (select string_agg(bound, '; ' order by lo) from bounds
