...
| `pgpm_archive/install.sql` | Optional archival add-on: archive aged partitions to S3 before retention drops them, config-driven (`archive.config`: boundary rule, drop-trigger rule, format, compression -- no "edit these N constants"). Graduates `docs/archive-*.md`'s embedded SQL into one installable module; those pages stay the narrative |
