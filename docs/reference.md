...
honored only while at least `ceil(obtain / 2)` complete partitions remain ahead of the frontier's own
partition: with no `DEFAULT` to catch a write past the grid, a back-off that outlasted the lookahead would
