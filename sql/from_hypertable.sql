...
-- brief lock. Scope of this first cut: a single time/RANGE dimension, a hypertable
-- whose control column is already part of a PRIMARY KEY or UNIQUE constraint (the
-- key pgpm reuses), append-only catch-up at cutover. Refused up front: continuous
-- aggregates, space partitioning (>1 dimension), and a keyless hypertable (no PK
-- or unique constraint on the control column -- the common shape, pending the
-- keyless path). Identity/sequence continuity and a trigger-based delta for
-- update/delete workloads are follow-ups.
-- =============================================================================
